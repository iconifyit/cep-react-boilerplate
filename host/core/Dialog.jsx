!(function(global, Observable, extend) {
    /**
     * Dialog class.
     * @param settings
     * @param logger
...
// dialog.sizePanel = dialog.add('panel',  [p1, 16, p2, 170],   Strings.LABEL_SIZE);
    ...
    Dialog.prototype.add = function(options) {
        try {
            ...
            if (this.panelHitTest(options) && type !== FieldTypes.PANEL) {
                this.logger.info('[Dialog.jsx] Hit test found, adjusting positions');
            ...
            if (type === FieldTypes.PANEL) {
                this.logger.info('[Dialog.jsx] Panel found, adjusting positions');
        ...
        catch(e) { this.logger.error('[Dialog.jsx]', e); throw e; }
    }
        ...
        try {
            ...
            for (var i = 0; i < (panels || []).length; i++) {
                ...
                if (f_column.between(p_column, (p_column + (p_span - 1)), true)) {
                    xhit = true;
                    this.logger.info('[Dialog.jsx] X Hit found');
                }

                if (f_row.between(p_row, (p_row + (p_rowspan - 1)), true)) {
                    yhit = true;
                    this.logger.info('[Dialog.jsx] Y Hit found');
                }
    ...
    /**
     * Extend Dialog with Observable prototype.
     */
    extend(Observable, Dialog);

    /**
     * Add Dialog to global scope.
     * @type {Dialog}
     */
